  ...
  try {
    ...
    const sql = `
      SELECT 
        ...
        u.username
      FROM SOP s
      LEFT JOIN SOPCategory c 
        ON s.category_id = c.category_id
      LEFT JOIN ${authDbName}.User u 
    ...
    if (sopIds.length > 0) {
      ...
      console.log(
        `[getAllSops] Retrieved ${attachments.length} attachments`
      );
...
exports.addSop = async (req, res) => {
  try {
    ...
    // Expect frontend to send the auth user_id from shared Auth DB.
    // Fallback to 1 for safety if missing (but better to always send).
    ...
    const insertSql = `
      INSERT INTO SOP 
        (title, content, version, effective_date, created_by_user_id, status, department_id, category_id)
      VALUES 
        (?, ?, '1.0', NULL, ?, 'draft', 1, ?)
    `;
    ...
    if (req.file) {
      ...
      const [attResult] = await pool.query(
        ...
        INSERT INTO Attachment 
          (parent_type, parent_id, filename, storage_path, uploaded_by_user_id)
        VALUES 
          ('sop', ?, ?, ?, ?)
        `,
  ...
  try {
    ...
    if (file) {
      ...
      const [oldFiles] = await pool.query(
        ...
        WHERE parent_type = 'sop' AND parent_id = ?
        `,
        [sop_id]
      );
      ...
      console.log(
        "[updateSop] New attachment inserted:",
        attResult.insertId
      );
    }

    // Fetch updated SOP with username
...
exports.deleteSop = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "SOP not found" });
    }

    console.log(
      "[deleteSop] SOP deleted successfully:",
      sop_id
    );
    res.status(200).json({
      message: "SOP and related files deleted successfully",
    });
  } catch (err) {
...
exports.addCategory = async (req, res) => {
  ...
  if (!category_name) {
    return res
      .status(400)
      .json({ message: "Missing category name" });
  }

  try {
    await pool.query(
      `
      INSERT INTO SOPCategory (category_name) 
      VALUES (?)
      `,
      [category_name]
    );
...
exports.uploadAttachment = async (req, res) => {
  const { sop_id } = req.params;
  const uploaded_by_user_id = 1; // ideally from logged-in user
  ...
  try {
    ...
    console.log(
      "[uploadAttachment] Attachment inserted:",
      result.insertId
    );
    ...
    console.error(
      "[uploadAttachment] Error uploading attachment:",
      err
    );
    res.status(500).json({ message: "Database error", error: err });
  }
...
exports.getSopFiles = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[getSopFiles] Error fetching SOP files:",
      err
    );
    res.status(500).json({ message: "Database error", error: err });
  }
